...
class LFUCache:
    ...
    def get(self, key: int) -> int:
        ...
        val = self.freqkeys[freq][key] #find put the value of the key that match the frequency
        del self.freqkeys[freq][key] #the freq will be +1, so remove it
    ...
    def put(self, key: int, value: int) -> None:
        ...
        if key in self.keyfreq:
            ...
            self.get(key)
            return
        if self.limit==len(self.keyfreq):
